...
describe("SpaceCoin", () => {
  ...
  describe("Tax functionality", () => {
    ...
    it("Takes 2% tax on transfers when taxes on", async () => {
      ...
      await tx.wait();
      tx = await ico.advancePhase(BigNumber.from(0));
      await tx.wait();
      tx = await ico.advancePhase(BigNumber.from(1));
      await tx.wait();
    ...
    it("Disallows non-owner to mint", async () => {
      const { spaceCoin, deployer, alice } = await loadFixture(setupFixture);
      await expect(
        spaceCoin
          .connect(deployer)
          .mint(alice.address, ethers.utils.parseEther("100"))).to.be.reverted;
    });
  });
});
